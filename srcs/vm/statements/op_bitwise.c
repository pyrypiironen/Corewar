...
void	op_and(t_carriage *carriage, t_vm_data *d)
{
	...
	arg_3 = d->arena[carriage->cursor_copy];
...
01 10 01 00 REG DIR REG == 0x64 == 1 + 4 + 1
01 11 01 00 REG IND REG == 0x74 == 1 + 2 + 1
10 01 01 00 DIR REG REG == 0x94 == 4 + 1 + 1
10 10 01 00 DIR DIR REG == 0xa4 == 4 + 4 + 1
10 11 01 00 DIR IND REG == 0xb4 == 4 + 2 + 1
11 01 01 00 IND REG REG == 0xd4 == 2 + 1 + 1
11 10 01 00 IND DIR REG == 0xe4 == 2 + 4 + 1
11 11 01 00 IND IND REG == 0xf4 == 2 + 2 + 1
*/
...
static long long get_first_arg(t_carriage *carriage, t_vm_data *d)
{
		...
		|| res == 0x74) && is_valid_reg(pos, d))
	{
		...
		return (carriage->registrys[d->arena[pos]]);
	}
		
	else if (((res == 0x94 && is_valid_reg((pos + 1) % MEM_SIZE, d)) \
		|| res == 0xa4 || res == 0xb4))
	{
	...
	else if (((res == 0xd4 && is_valid_reg((pos + 1) % MEM_SIZE, d)) \
		|| res == 0xe4 || res == 0xf4))
	{
		carriage->cursor_copy = (carriage->cursor + 4) % MEM_SIZE;
		return (get_4_byte_value(d, get_2_byte_value(d, pos, 1)));
	}
	return (2147483648);
}
...
static long long get_second_arg(t_carriage *carriage, t_vm_data *d)
{
	...
	{
		carriage->cursor_copy = (carriage->cursor + 1) % MEM_SIZE;
		return (carriage->registrys[d->arena[pos]]);
	}
	...
	{
		carriage->cursor_copy = (carriage->cursor + 2) % MEM_SIZE;
		return (get_4_byte_value(d, get_2_byte_value(d, pos, 1)));
	}
	return (2147483648);
}