...
void	read_magic(int fd)
{
	...
	if (read(fd, &buf, 4) != 4)
		print_error("Invalid file. .cor file is too short. [Magic code]", 0);
	total = (buf[0] << 24) + (buf[1] << 16) + (buf[2] << 8) + buf[3];
	if (total != COREWAR_EXEC_MAGIC)
		print_error("Invalid file. Magic code is invalid.", 0);
}
...
void	read_name(t_player *champ, int fd)
{
	if (read(fd, champ->name, PROG_NAME_LENGTH) != PROG_NAME_LENGTH)
		print_error("Invalid file. .cor file is too short. [Name]", 0);
...
void	skip_nulls(int fd)
{
	...
	i = 0;
	if (read(fd, &buf, 4) != 4)
		print_error("Invalid file. .cor file is too short. [NULLs]", 0);
	while (i < 4)
	{
		if (buf[i])
			print_error("Invalid file.  No NULLs on right place.", 0);
		i++;
	}
}
